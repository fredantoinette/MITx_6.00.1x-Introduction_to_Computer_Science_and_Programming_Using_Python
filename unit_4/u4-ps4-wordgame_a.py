...
value.
- For example, 'weed' would be worth 32 points ((4+1+1+2) for the four 
letters, then multiply by len('weed') to get (4+1+1+2)*4 = 32). Be sure to 
check that the hand actually has 1 'w', 2 'e's, and 1 'd' before scoring the 
word!
...
(4+1+4+3+1+1+1)*7=105, plus an additional 50 point bonus for using all n 
letters).
"""
...
def updateHand(hand, word):
    ...
    for w in word:
        updated_hand[w] = updated_hand.get(w, 0) - 1
    return updated_hand

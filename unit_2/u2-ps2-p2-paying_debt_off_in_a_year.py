...
def lowest_payment(balance, annualInterestRate):
    min_fixed_monthly_payment = 0
    updated_bal = balance
    monthly_int_rate = annualInterestRate / 12.0
    while updated_bal > 0:
        updated_bal = balance
        min_fixed_monthly_payment += 10
        for m in range(12):
            monthly_unpaid_bal = updated_bal - min_fixed_monthly_payment
            updated_bal = monthly_unpaid_bal + monthly_int_rate * monthly_unpaid_bal
    return round(min_fixed_monthly_payment, 0)

#print("Lowest Payment:", lowest_payment(balance, annualInterestRate))

# test
print("Lowest Payment:", lowest_payment(3329, 0.2))
print("Lowest Payment:", lowest_payment(4773, 0.2))
print("Lowest Payment:", lowest_payment(3926, 0.2))
